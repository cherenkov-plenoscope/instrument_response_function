...
def export_effective_area(
    ...
    acceptance [sr*cm^2]. Crucial instrument and simulation settings are
    machine written into the text file to keep track of the results.
    This output is intended for the high level analysis bridge
...
def make_effective_area(
    ...
    Aeff = make_effective_area_histogram(
        primary_particle_energies=acp_event_responses[
...
def make_effective_area_report(effective_area):
    ...
    out += '#     threshold: '
    out += float2str(effective_area['detector_response_threshold'])+'\n'
    out += '#\n'
